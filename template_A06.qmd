...
[https://github.com/laurenryoo619/Ryoo_Lauren_201_A06](https://github.com/laurenryoo619/Ryoo_Lauren_201_A06)
...
```{r}
# using indexing
indexing <- player_info[[2]][["points"]]
print(indexing)

# using names
names <- player_info[["LJ"]][["points"]]
print(names)
```
...
```{r}
ppg_vector <- sapply(ppg_list, round, 2)
...
round_values <- function(x) {
  if (x < 100) {
    return(round(x, -1)) # rounding to the nearest tens
  } else if (x < 1000) { 
    return(round(x, -2)) # rounding to the nearest hundreds
  } else {
    return(round(x, -3)) # rounding to the nearest thousands 
  }
}

# applying the function to each element in the matrix
mat_rand2 <- apply(mat_rand, MARGIN = 1:2, round_values)
...
```{r} 
temp_avg <- apply(max_temps[, 3:14], 2, mean)
print(temp_avg)
```
...
```{r}
# slicing the data frame for Colorado
colorado_data <- max_temps[max_temps$state == "CO",]

# slicing the data frame for Ohio
ohio_data <- max_temps[max_temps$state == "OH",]

# slicing the data frame for Virginia
...
```{r}
# calculating the median annual temperature from the Colorado dataset
colorado_median_annual <- apply(colorado_data[, 3:14], 1, median)

# finding the years with the highest and lowest median temperatures
max_median_temp <- max(colorado_median_annual)
min_median_temp <- min(colorado_median_annual)
max_median_index <- which.max(colorado_median_annual)
min_median_index <- which.min(colorado_median_annual)

# finding the corresponding years 
max_median_year <- colorado_data$year[max_median_index]
min_median_year <- colorado_data$year[min_median_index]
...
sum_of_digits <- function(n) {
  #base case: returning n if the number is a single digit
  if (n < 10) {
    return(n)
  } else {
